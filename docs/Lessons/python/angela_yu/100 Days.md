Day 100 Days of Python - Udemy - Angela Yu P1. 

Lecture 3 - contains links to course resources. Exercises on repl. it - fork to modify and run the assignments. She recommends dark mode & large font on repl. it.
...
Editor shows errors by color high lighting. 
...
Con't concatenate int to string: 

Print ("Your Name has_str (len (name)) + "_cha) convert to str, float 

+, -, *, /, ** 

division always results in float 

parantheses, exponets, (mult, div LTOR), (+,- LTOR) 
...
f "your score is score" even if score is int. print (f "your score is & score}, your height is { & height 3, you are winning is {is Winning?") in an f string :.x behind on a float displays x characters after the decimal point. print (f" Earth person should pay Ɛanswer:. 243") 
...
She used draw, io to create a flow chart 
...
AND, OR NOT (NOT CAPITALIZED) 

Case 

= 

name, lower () - changes string name to all lower 

name- lower = name, lower () name, count ("a") - counts # of a's in name 

num_a = name, count ("a") 

multiline strings start & end with three 

single quotes 

askpython.com search for python stuff 

RANDOM import random. 

41, LISTS 

random-integer = random.randint (1, 10) create a module called my-module.py 
...
Starter code 3 files, main (empty) art (logo & vs), game-data (list of dictionary entries), Keys: name, follower count, description, country 
